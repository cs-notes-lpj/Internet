...
- 其他要发送数据的无线站点，还需退避一段**随机**时间后才能发送

> 思考：
>
> 既然信道已经由忙转为空闲，且经过了帧间间隔 DIFS，那为什么还要再退避一段**随机**时间才能使用信道而不是立即使用信道呢 ？
>
...
- 在执行退避算法时，站点为退避计时器设置一个随机的退避时间
	...
	- 当退避计时器的时间还未减小到 0 而信道又转变为忙状态，则冻结退避计时器

	- 待到信道重新变为空闲，再经过帧间间隔 DIFS 后，继续启动退避计时器

---

- 在进行第 i 次退避时，在时隙编号{0，1，...，2^(2+i) - 1}中随机选择一个

- 再乘以基本退避时间（一个时隙的长度）就可以得到随机的退避时间

- 这样做是为了降低不同站点选择相同退避时间的概率
...
![image-20220323122028399](https://gitee.com/pj-l/imgs-1/raw/master/image-20220323122028399.png)

- 源站在发送数据帧之前会先发送一个短的控制帧，称为请求发送 RTS（`R`equest `T`o `S`end），它包括源地址、目的地址以及这次通信（包括相应的确认帧）所需的持续时间

- 如图所示，当源站检测到信道空闲，且经过一个帧间间隔 DIFS 后，发送 RTS 帧

- 若目的站正确接收到源站发来的 RTS 帧，在检测到信道空闲，且经过帧间间隔 SIFS 后，目的站就会发送一个响应控制帧，称为允许发送 CTS（`C`lear `T`o `S`end）

- RTS 帧中包含的此次通信所需的持续时间会被复制到 CTS 帧中

- 源站收到 CTS 帧后，再等待一个帧间间隔 SIFS，就可发送其数据帧

- 若目的站正确收到了源站发来的数据帧，再等待一个帧间间隔 SIFS，就向源站发送确认帧 ACK

---

- 除源站和目的站以外的其它各站，在收到 CTS 帧或数据帧后，就会推迟接入到无线局域网，这样就保证了源站和目的站之间的通信不会受到其它站的干扰，如下图所示

<img src="https://gitee.com/pj-l/imgs-1/raw/master/image-20220323122509946.png" alt="image-20220323122509946" style="zoom:55%;" />

- 如果 RTS 帧发生碰撞，源站就收不到 CTS 帧，就需要执行退避算法重传 RTS 帧

- 由于 RTS 帧和 CTS 帧很短，故其（本身的开销、发生碰撞的概率、碰撞产生的开销）都很小

- 而对于一般的数据帧

	- 发送时延往往大于传播时延，且碰撞的概率很大

	- 一旦发生碰撞而导致数据帧重发，则浪费的时间就很多

- 因此，用很小的代价对信道进行预约往往是值得的
...
- 802.11 标准规定了 3 种情况供用户选择
	...
	3. 只有当数据帧的长度超过某一数值时，才使用 RTS 帧和 CTS 帧

---

> 除 RTS 帧和 CTS 帧会携带通信需要持续的时间，数据帧也能携带通信需要持续的时间，这称为 802.11 的虚拟载波监听机制

- 由于虚拟载波监听机制，站点只需监听到（RTS 帧、CTS 帧、数据帧）中的任何一个，就能知道信道被占用的持续时间，而不需要真正监听到信道上的信号

- 如下图是 4 个无线站点，A 和 C 互为隐蔽站

<img src="https://gitee.com/pj-l/imgs-1/raw/master/image-20220323141600941.png" alt="image-20220323141600941" style="zoom:50%;" />

- A 在给 B 发送数据帧之前，可以使用 RTS 帧来预约信道

- 尽管 C 收不到该 RTS 帧，但是 C 可以收到 B 发送给 A 的 CTS 帧，这样 C 就知道了信道将被占用多长时间，在这段时间内 C 都不会争用信道

- 也就是说，A 给 B 发送数据帧时不会受到 C 的干扰

- 所以说：虚拟载波监听机制能减少隐蔽站带来的碰撞问题

#### 习题

![image-20220323141857715](https://gitee.com/pj-l/imgs-1/raw/master/image-20220323141857715.png)

![image-20220323142031243](https://gitee.com/pj-l/imgs-1/raw/master/image-20220323142031243.png)

![image-20220323142128485](https://gitee.com/pj-l/imgs-1/raw/master/image-20220323142128485.png)
