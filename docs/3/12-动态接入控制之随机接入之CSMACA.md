...
- 由于 RTS 帧和 CTS 帧很短，发生碰撞的概率、碰撞产生的开销以及本身的开销都很小

- 而对于一般的数据帧

	- 在局域网中，其发送时延往往大于传播时延，碰撞的概率很大

	- 且一旦发生碰撞而导致数据帧重发，则浪费的时间就很多
...
- 由于利用虚拟载波监听机制，站点只需监听到（RTS 帧、CTS 帧、数据帧）中的任何一个，就能知道信道被占用的持续时间，而不需要真正监听到信道上的信号
